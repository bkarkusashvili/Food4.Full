<template>
  <div>
    <h1 class="title">მომხმარებლის რეცეპტები</h1>
    ...
    <table class="table is-striped is-hoverable is-fullwidth">
      ...
      <tbody>
        <tr v-for="post in posts" :key="post._id">
          ...
          <td>{{(post.publishedAt || post.createdAt) | dateTime}}</td>
          <td>
...
export default {
  ...
  methods: {
    ...
    archivePost: function(post) {
      ...
      this.$axios
        .put("/api/admin/user-recipes/" + post._id, {
          status: "archived"
        })
        .then(response => {
          this.$notifySuccess({
    ...
    removePost: function(post) {
      ...
      this.$axios
        .delete("/api/admin/user-recipes/" + post._id)
        .then(response => {
          this.$notifySuccess({
    ...
    fetchData: function() {
      this.$axios
        .get("/api/admin/user-recipes", { params: this.queryParams() })
        .then(response => {
