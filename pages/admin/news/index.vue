<template>
  <div>
    <h1 class="title">სიახლეები</h1>

    <form class="form">
      <div class="field is-grouped">
        <div class="control">
          <nuxt-link to="/admin/news/new" class="button is-success">
            <span class="icon">
    ...
    <table class="table is-striped is-hoverable is-fullwidth">
      ...
      <tbody>
        <tr v-for="post in posts" :key="post._id">
          <td>
            <a :href="post.link || ('/news/' + post.slug)" target="_blank">{{post.title || post.slug}}</a>
          </td>
          <td>
            ...
            <div class="dropdown is-hoverable is-right">
              ...
              <div class="dropdown-menu" id="dropdown-menu" role="menu">
                <div class="dropdown-content">
                  <nuxt-link :to="'/admin/news/' + post._id" class="dropdown-item">
                    <span class="icon">
...
export default {
  ...
  methods: {
    ...
    archivePost: function(post) {
      if (!confirm("ნამდვილად გსურთ პოსტის დაარქივება?")) return;
      this.$axios
        ...
        .then(response => {
          this.$notifySuccess({
            title: "პოსტი დაარქივებულია",
            text: "პოსტი წარმატებით დაარქივდა!"
          });
          this.fetchData();
        })
        .catch(err => {
    ...
    publishPost: function(post) {
      if (!confirm("ნამდვილად გსურთ პოსტის გამოქვეყნება?")) return;
      this.$axios
        .put("/api/admin/posts/" + post._id, {
          status: "published",
        ...
        .then(response => {
          this.$notifySuccess({
            title: "პოსტი გამოქვეყნებულია",
            text: "პოსტი წარმატებით გამოქვეყნდა!"
          });
          this.fetchData();
        })
        .catch(err => {
    ...
    unpublishPost: function(post) {
      if (!confirm("ნამდვილად გსურთ პოსტის გამოქვეყნების გაუქმება?")) return;
      this.$axios
        .put("/api/admin/posts/" + post._id, {
          status: "draft"
        })
        .then(response => {
          this.$notifySuccess({
            title: "პოსტი გამოქვეყნებიდან მოხსნილია",
            text: "პოსტი წარმატებით მოიხსნა გამოქვეყნებულებიდან!"
          });
          this.fetchData();
        })
        .catch(err => {
    ...
    removePost: function(post) {
      ...
      this.$axios
        ...
        .then(response => {
          this.$notifySuccess({
            title: "პოსტი წაშლილია",
            text: "პოსტი წარმატებით წაიშალა!"
          });
          this.fetchData();
        })
        .catch(err => {
